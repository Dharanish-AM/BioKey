...
export default function RecycleBin({ navigation }) {
  ...
  const binFiles = useSelector(
    (state) => state.files.recycleBinFiles,
    shallowEqual,
  );
      ...
      if (item.type === "audios") {
        return (
          <View style={styles.customThumbnailContainer}>
            <Image
              source={
                item.thumbnailUrl ? { uri: item.thumbnailUrl } : AudioFileIcon
              }
              style={styles.fileImage}
            />
          </View>
    ...
    return (
      <View style={styles.fileContainer}>
        <View style={styles.fileInfo}>
          <View style={styles.fileThumbnail}>{renderThumbnail()}</View>
          <View style={styles.fileDetails}>
            <View style={styles.fileMeta}>
              <Text style={styles.fileName}>
                {item.name} <Text style={{ color: colors.textColor2 }}>-</Text>{" "}
                <Text style={styles.fileSize}>{formatFileSize(item.size)}</Text>
              </Text>
            </View>
            <Text style={styles.fileDate}>
              {new Date(item.deletedAt).toUTCString()}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.modalOverlay}>
          ...
          >
            ...
            >
              ...
              <TouchableOpacity
                onPress={() => handleDeleteFileOne(selectedFile)}
              >
                <Text
                  style={[
