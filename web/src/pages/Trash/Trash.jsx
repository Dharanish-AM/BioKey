...
export default function Trash() {
  ...
  const sortedFiles = [...TrashFiles].sort(
  ...
  const handleClearTrash = async (isAll = false) => {
    const confirmation = confirm(
      `Are you sure you want to delete ${isAll ? "all files" : "the selected files"}?`,
    );
    ...
    if (confirmation) {
      ...
      const apiResponse = await permanentDelete(
        userId,
        fileIds,
        isAll,
        token,
  ...
  const handleRestoreTrash = async (isAll = false) => {
    const confirmation = confirm(
      `Are you sure you want to restore ${isAll ? "all files" : "the selected files"}?`,
    );
    ...
    try {
      const apiResponse = await restoreFile(
        userId,
        fileIds,
        type,
        token,
    ...
    <div className="trash-container">
      <div className="trash-header">
        ...
        <div className="trash-options-container">
          ...
          >
            {isMultiSelect ? (
              <X color="var(--text-color2)" size={"1.7rem"} />
            ) : (
              <TbCheckbox color="var(--text-color2)" size={"1.9rem"} />
            )}
          </div>
          ...
          <div
            onClick={() => handleRestoreTrash(isMultiSelect ? false : true)}
            className="restore-trash-button"
          >
            {isMultiSelect ? "Restore Selected" : "Restore All"}
          </div>
          <div
            onClick={() => handleClearTrash(isMultiSelect ? false : true)}
