...
import { Search, X, SlidersHorizontal, Plus, Eye, EyeOff, Copy } from "lucide-react";
import { useDispatch, useSelector } from "react-redux";
import { getAllPasswords, getPasswordBreachStatus } from "../../../services/passwordOperations";
      ...
      <div className="passwords-container-header">
        ...
        <div className="passwords-container-header-options">
          ...
          <div className="add-passwords-btn" onClick={handleAddPassword}>
            <Plus size="1.5rem" color="var(--text-color3)" /> <span>|</span> Add Password
          </div>
        </div>
      </div>
          ...
          {passwords?.map((password) => (
            <div key={password._id} className="passwords-list-item">
              ...
              <TbEdit
                ...
                size="1.7rem"
                style={{ stroke: "var(--text-color2)", strokeWidth: "1.3", position: "absolute", top: "1rem", right: "1rem", cursor: "pointer" }}
              />
              {breachStatuses[password._id]?.breached ? (
                <span className={`passwords-list-item-breach-status breached`}>Your password has been breached in {breachStatuses[password._id]?.breachCount} places.</span>
              ) : (
                <span className="passwords-list-item-breach-status notBreached">Your password is safe.</span>
              )}
            </div>
          ))}
        </div>
