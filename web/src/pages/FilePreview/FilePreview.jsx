...
import { FiDownload, FiFolderMinus, FiMoreVertical } from "react-icons/fi";
import {
  FiHeart,
  FiFolderPlus,
  FiExternalLink,
...
export default function FilePreview({ file, onClose }) {
  ...
  const handleAddToFavorites = async () => {
    ...
    const response = await likeOrUnlikeFile(
      userId,
      file._id,
      token,
      file.type,
  ...
  const handleMoveFileToFolder = async (folderId) => {
    try {
      ...
      const response = await handleFolderMove(
        userId,
        folderId,
        file._id,
        token,
  ...
  const handleRemoveFromFolder = async () => {
    ...
    if (response.success) {
      ...
      toast.error(`Failed to remove ${file.name} from folder`);
    }
  };
          ...
          {isAddToFolder ? (
                ...
                folders.map((folder, index) => {
                  ...
                  return (
                    <div
                      ...
                      onClick={
                        !fileExists
                          ? () => handleMoveFileToFolder(folder.folderId)
                          : undefined
                      }
                    >
                      📁 {folder.folderName}
                      {fileExists && (
                        <span className="file-exists"> (Already Exists)</span>
                      )}
                    </div>
                  );
                })
            ...
            <div className="file-more-info-options">
              <div className="file-more-info-options-row">
                ...
                <div className="file-more-info-options-value name">
                  {file.name}
                </div>
              </div>
              <div className="file-more-info-options-row">
                <div className="file-more-info-options-label">Created At:</div>
                <div className="file-more-info-options-value">
              ...
              <div className="file-more-info-options-row">
                ...
                <div
                  className={`file-more-info-options-value ${
            ...
            <>
              ...
              {folderId ? (
                <div className="file-option" onClick={handleRemoveFromFolder}>
                  <FiFolderMinus size={"1.2rem"} className="file-option-icon" />
                  Remove from folder
                </div>
              ) : (
                <div
                  className="file-option"
                  onClick={() => setIsAddToFolder(true)}
                >
  ...
  return (
    ...
    >
      ...
      >
        <source
          src={fileData.url}
          type={fileData.contentType || "audio/mpeg"}
        />
